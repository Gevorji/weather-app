...
будут прочитаны в соответствующие переменные модуля настроек проекта Django
## Переменные в settings.py, уникальные для проекта
В файле настроек Django проекта settings.py могут быть определены следующие переменные:
- `SAVE_LOCATIONS_ONLY_WHEN_ADDED_BY_USER` - тип bool, по умолчанию - True. True заставляет приложение сохранять локацию 
в БД только после добавления ее пользователем. Значение False означает, 
что в базу данных будут сохраняться все отсутствующие в ней локации сразу после поиска. 
Здесь "локация" не означает локация пользователя, так как связь между пользователем и локацией реализована в отдельной 
таблице БД.
- `NLOCATIONS_ON_PAGE` - тип int, указывает, сколько локаций пользователя отображать на главной странице при пагинации.
...
После запуска контейнеров будет создана чистая база данных. Для работы приложения необходимо выполнить
